...
abstract class Skin_Style {
    ...
    public function render_title(){
        ...
        if( 'yes' === $link_title ){
            echo '<'.esc_attr($tag).' class="blfe-post-list-title">
            <a href="' . esc_url(get_permalink()) . '" title="' . esc_attr(get_the_title()) . '" rel="nofollow" target="'.esc_attr($target).'">' . esc_html($title)  . '</a>
            </'.esc_attr($tag).'>';
        } else {
            echo '<'.esc_attr($tag).' class="blfe-post-list-title">'. esc_html($title) .'</'.esc_attr($tag).'>';
        }
    }
    ...
    public function render_image($thumbnail_id, $image_size){
        ...
        if( 'yes' === $link_img ) {
            echo '<div class="blfe-post-image-wrapper"><a class="blfe-post-list-image" href="' . esc_url(get_permalink()) . '" title="' . esc_attr(get_the_title()) . '" rel="nofollow" target="'.esc_attr($target).'"><span style="background-image: url('.esc_url($image_src).')"></span></a></div>';
        } else {
            echo '<div class="blfe-post-image-wrapper"><div class="blfe-post-list-image"><span style="background-image: url('.esc_url($image_src).')"></span></div></div>';
        }
    }
    ...
    public function render_excerpt() {
        ...
        echo '<p class="blfe-post-list-excerpt">' . esc_html(Helper::custom_excerpt($excerpt_length, $indicator)) . '</p>';
    }
    ...
    public function render_author(){
        ...
        echo '<span class="blfe-post-list-author"><i class="fa fa-user far fa-user"></i>'.esc_html__('by ', 'bloglentor').'<a href="'.esc_url(get_author_posts_url(get_the_author_meta( 'ID' ))).'">'.esc_html(get_the_author()).'</a></span>';
    }
    ...
    public function render_date(){
       // $icon = $this->get_instance_value('meta_style') !== 'no_icon' ? '<i class="fa fa-calendar-alt far fa-calendar-alt"></i>' : '';
        ...
        echo '<span class="blfe-post-list-date"><i class="fa fa-calendar-alt far fa-calendar-alt"></i>'.esc_html(apply_filters( 'the_date', get_the_date(), get_option( 'date_format' ), '', '' )).'</span>';
    }

    public function render_time(){
        //$icon = $this->get_instance_value('meta_style') !== 'no_icon' ? '<i class="fa fa-clock far fa-clock"></i>' : '';
        echo '<span class="blfe-post-list-time"><i class="fa fa-clock far fa-clock"></i>'.esc_attr(get_the_time()).'</span>';
    }
    ...
    public function render_meta(){
        ...
        <div class="blfe-post-list-meta">
            ...
            if ( in_array( 'reading_time', $this->get_instance_value('meta_data') ) ) {
                ...
                echo '<span class="blfe-post-list-time"><i class="fas fa-book-reader"></i>'.esc_html(Helper::reading_time()).'</span>';
            }
    ...
    protected function render_read_more() {
        ...
        ?>
        <a class="blfe-post-read-more" href="<?php echo esc_url($this->current_permalink); ?>" <?php echo esc_attr($optional_attributes_html); ?>>
            <span <?php echo wp_kses_post($this->get_render_attribute_string( 'content-wrapper'. get_the_ID() )); ?>>

             <span <?php echo wp_kses_post($this->get_render_attribute_string( 'text'. get_the_ID() )); ?>><?php echo esc_html($this->get_instance_value( 'read_more_text' )); ?></span>
