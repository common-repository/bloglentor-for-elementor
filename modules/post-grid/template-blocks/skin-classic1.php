...
class Skin_Classic1 extends Skin_Style {
    ...
    public function render_image($thumbnail_id, $image_size){
        ...
        if( 'yes' === $link_img ) {
            echo '<a class="blfe-post-grid-image" href="' . esc_url(get_permalink()) . '" title="' . esc_attr(get_the_title()) . '" rel="nofollow" target="'.esc_attr($target).'"><img src="'.esc_url($image_src).'" alt="'.esc_attr(get_the_title()).'"/></a>';
        } else {
            echo '<div class="blfe-post-grid-image"><img src="'.esc_url($image_src).'" alt="'.esc_attr(get_the_title()).'"/></div>';
        }
